                        ...
                        <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
                            {data.auditLogs.length === 0 ? <p className="text-zinc-500 italic">No hay registros de seguridad recientes.</p> : 
                              data.auditLogs.map((log: any) => (
                                <div key={log.id} className="bg-zinc-950 p-4 rounded-2xl border border-zinc-800 flex flex-col md:flex-row md:justify-between md:items-center gap-3 text-xs">
                                    <div className="flex flex-col md:flex-row gap-2 md:gap-4 md:items-center">
                                        <span className="text-zinc-600 font-bold whitespace-nowrap">{new Date(log.createdAt).toLocaleString()}</span>
                                        <span className="bg-zinc-900 px-3 py-1.5 rounded-lg text-yellow-500 font-black uppercase text-[10px] tracking-widest border border-zinc-800 self-start md:self-auto">{log.action}</span>
